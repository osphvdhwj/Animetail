...
class MangaBakaApi(
    ...
    private val client = baseClient.newBuilder().addInterceptor {
        it.request().newBuilder()
            .header(
                "User-Agent",
                buildString {
                    append("Animetail/v${BuildConfig.VERSION_NAME} ")
                    append("(${BuildConfig.APPLICATION_ID} ${BuildConfig.COMMIT_SHA}) ")
                    append("(Android) (https://github.com/Animetailapp/Animetail)")
                },
            )
            .build()
            .let(it::proceed)
        ...
        return withIOContext {
            with(json) {
                try {
                    ...
                    MangaTrack.create(TrackerManager.MANGABAKA).apply {
    ...
    companion object {
        private const val CLIENT_ID = "qHfLySYDKrngmdqtuUZOvUCicAVybGeT"
        ...
        private const val REDIRECT_URI = "animetail://mangabaka-auth"
