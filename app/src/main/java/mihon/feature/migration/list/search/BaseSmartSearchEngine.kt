...
abstract class BaseSmartSearchEngine<T>(
    ...
    protected suspend fun deepSearch(searchAction: SearchAction<T>, title: String): T? {
        ...
        return baseSearch(searchAction, queries) {
            val cleanedTitle2 = cleanDeepSearchTitle(getTitle(it))
            normalizedLevenshtein.similarity(cleanedTitle, cleanedTitle2)
        }
    }
    ...
    private fun removeTextInBrackets(text: String, readForward: Boolean): String {
        ...
        return buildString {
            val chars = if (readForward) text.toCharArray() else text.reversed().toCharArray()
            for (char in chars) {
                when (char) {
                    ...
                    // Avoid depth going negative on mismatched closing
                    else -> if (depth == 0) {
    ...
    companion object {
        ...
        private val chapterRefCyrillicRegexp = Regex("""((- parte|- глава) \d*)""")
    }
}
