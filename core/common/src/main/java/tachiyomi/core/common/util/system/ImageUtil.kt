...
object ImageUtil {
    ...
    fun decodeBitmap(stream: InputStream): Bitmap? {
        return try {
            ...
            logcat(LogPriority.ERROR) { "decodeBitmap failed: ${e.message}" }
            null
        }
    }
    ...
    fun addHorizontalCenterMargin(
        ...
        val imageBitmap = decodeBitmap(imageSource.peek().inputStream()) ?: return imageSource
        val height = imageBitmap.height
        val width = imageBitmap.width
    ...
    fun chooseBackground(context: Context, imageSource: BufferedSource): Drawable {
        return chooseBackground(context, imageSource.peek().inputStream())
    }
